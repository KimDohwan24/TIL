...
description: Delta 조회, SSR 프리페치, ISR로 주식 전체조회 API 워터폴과 중복 요청을 줄인 과정.
categories: TIL
...
30초간 캐시된 페이지를 제공한다. 종목 목록의 변경 주기와 최신성 요구를 확인한 뒤 30초라는 값을 선택했다.
...
프로젝트의 린트 규칙과 상태 전이 요구를 함께 만족하는 방식으로 동작한다. 다만 reducer를 쓴다는 이유만으로 렌더 단계의 상태 변경이 항상 안전해지는 것은 아니므로, dispatch 위치와 의존성은 별도로 점검해야 한다.
...
| 호출 경로 검토 | CORS로 브라우저 직접 호출을 허용하기 전에 인증·보안·캐시 정책과 Next.js 프록시의 필요성을 함께 검토 |
...
4. **ISR로 SSR 부하 감소**: `force-dynamic`은 매 요청마다 백엔드 호출한다. 실시간성이 덜 중요한 목록 페이지는 최신성 요구를 확인한 뒤 `revalidate`를 적용할 수 있다.

## 후속 글

카탈로그와 시세를 서버에서 통합하고, 외부 API·커넥션 풀·SWR까지 확장해 검토한 내용은 [주식 조회 서비스 성능 최적화]({{ '/til/2026/05/19/TIL-36.html' | relative_url }})에서 이어서 정리했다.